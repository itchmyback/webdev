...
function App() {
  ...
  // Hook this app up to our backend or database
  let url =  'https://accsoftwarebootcamp-todos.herokuapp.com/todos';

  // call the API
  // useEffect (promise format)
//   useEffect( () => { fetch( url).then().catch()}, [] )
// fetch response, then parse with json()
/* this worked
useEffect( () => {
	fetch( url )
	.then( res => res.json() )
	...
	// react will complain about the empty dependency array below
	// let's temporary comment out
// }), []
})
*/
...
useEffect( () => {
	...
	.then( res => res.json() )
	.then( data => {
		// explanation here that is WTF
		setState( ( s ) => {
			// this notation with commas is multiple objects, but the latest overwrites
			return { // braces here represent an object, but wtf is this? a comma separate list of objects
				...s, todos: data
				// could add even more objects
			}
		})
	})
	.catch( err => {
...
// }), []
  ...
  const onclickMarkTodoComplete = ( id ) => {
	...
	/* can remove all this below due to fetch above
...
//   const onAddTodo = useCallback(event => {
   const onAddTodo = ( event ) => {
	...
	// upgrade for backend connect

	if( !state.newTodo ) return false;

	fetch( url, {
		method: 'POST',
		// what should description value be? from the new state
		// body: { description: state.newTodo }
		// lack of id or something?

		// appears to be an issue here in body? so try different formats?
